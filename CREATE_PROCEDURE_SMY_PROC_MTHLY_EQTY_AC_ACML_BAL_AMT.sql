CREATE PROCEDURE SMY.PROC_MTHLY_EQTY_AC_ACML_BAL_AMT(ACCOUNTING_DATE DATE)
-------------------------------------------------------------------------------
-- (C) Copyright ZJRCU and IBM <date>
--
-- File name:           SMY.PROC_MTHLY_EQTY_AC_ACML_BAL_AMT.sql
-- Procedure name: 			SMY.PROC_MTHLY_EQTY_AC_ACML_BAL_AMT
-- Source Table:				SOR.EQTY_AC_TXN_DTL,SOR.EQTY_AC_SUB_AR
-- Target Table: 				SMY.MTHLY_EQTY_AC_ACML_BAL_AMT
-- Project:             ZJ RCCB EDW
--
-- Purpose:             
--
...
END;
/*变量赋值*/
SET SMY_PROCNM = 'PROC_MTHLY_EQTY_AC_ACML_BAL_AMT';
...
SELECT MAX(ACG_DT) INTO MAX_ACG_DT FROM SMY.MTHLY_EQTY_AC_ACML_BAL_AMT;
SET DELETE_SQL='ALTER TABLE HIS.MTHLY_EQTY_AC_ACML_BAL_AMT ACTIVATE NOT LOGGED INITIALLY WITH EMPTY TABLE';
...
IF MAX_ACG_DT=ACCOUNTING_DATE THEN
   DELETE FROM SMY.MTHLY_EQTY_AC_ACML_BAL_AMT WHERE CDR_YR=CUR_YEAR AND CDR_MTH=CUR_MONTH;
   COMMIT;
   IF CUR_DAY<>1 THEN
      INSERT INTO SMY.MTHLY_EQTY_AC_ACML_BAL_AMT SELECT * FROM HIS.MTHLY_EQTY_AC_ACML_BAL_AMT;
      COMMIT;
   END IF;
ELSE
   EXECUTE IMMEDIATE DELETE_SQL;
   INSERT INTO HIS.MTHLY_EQTY_AC_ACML_BAL_AMT SELECT * FROM SMY.MTHLY_EQTY_AC_ACML_BAL_AMT WHERE CDR_YR=CUR_YEAR AND CDR_MTH=CUR_MONTH;
   COMMIT;
END IF;
...
DECLARE GLOBAL TEMPORARY TABLE TMP(EQTY_AC_AR_ID CHAR(20),
                                   DNMN_CCY_ID CHAR(3),
                                   CDR_YR SMALLINT,
                                   CDR_MTH SMALLINT,
                                   ACG_DT DATE,
...
ON COMMIT PRESERVE ROWS NOT LOGGED WITH REPLACE PARTITIONING KEY(EQTY_AC_AR_ID);
INSERT INTO SESSION.TMP
WITH TMP0 AS (
Select EQTY_AC_AR_ID,
       DNMN_CCY_ID,
...
GROUP BY EQTY_AC_AR_ID,
         DNMN_CCY_ID
), TMP_EQTY AS (
   SELECT  EQTY_AC_AR_ID
          ,DNMN_CCY_ID
          ,max(case when AR_LCS_TP_ID = 20370007     --正常 
                    then 1 else 0 end) NOD_In_MTH
          ,SUM(BAL_AMT) BAL_AMT
   FROM SOR.EQTY_AC_SUB_AR
   WHERE (
          END_DT >= YR_FIRST_DAY   --今年有效的记录
           OR
          END_DT = '1899-12-31'
          )
          AND DEL_F=0                     --未删除
...
FROM  TMP_EQTY A
LEFT JOIN TMP0 B
ON A.EQTY_AC_AR_ID=B.EQTY_AC_AR_ID AND A.DNMN_CCY_ID=B.DNMN_CCY_ID
---------------Start on 20100119---------------------
...
SET SMY_STEPNUM = SMY_STEPNUM + 1 ;
SET SMY_STEPDESC = 'UPDATE SMY.MTHLY_EQTY_AC_ACML_BAL_AMT.';

IF CUR_DAY=1 THEN                                                                     --月初
   IF CUR_MONTH IN (4,7,10) THEN                                                      --季初非年初
      INSERT INTO SMY.MTHLY_EQTY_AC_ACML_BAL_AMT(AC_AR_ID,
                                                 CCY,
                                                 CDR_YR,
                                                 CDR_MTH,
                                                 ACG_DT,
      ...
      FROM SESSION.TMP S
      LEFT JOIN SMY.MTHLY_EQTY_AC_ACML_BAL_AMT T
      ON S.EQTY_AC_AR_ID=T.AC_AR_ID
   ...
   END IF;
ELSE                                                                                          ---非月初
  MERGE INTO SMY.MTHLY_EQTY_AC_ACML_BAL_AMT S
  USING SESSION.TMP T
  ON S.AC_AR_ID=T.EQTY_AC_AR_ID 
